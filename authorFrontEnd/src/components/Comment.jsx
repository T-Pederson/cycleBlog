...
export default function Comment({ comment, authorId, refreshPost }) {
  const [editMode, setEditMode] = useState(false);
  const [editedComment, setEditedComment] = useState(comment.content);
  const [editedCommentErrors, setEditedCommentErrors] = useState([]);

  async function authorDeleteComment() {
    const res = await fetch(
      `${import.meta.env.VITE_API_URL}/comment/author/delete/${comment.id}`,
      {
        method: "POST",
        headers: {
